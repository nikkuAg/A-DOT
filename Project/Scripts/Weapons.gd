...
func _ready() -> void:
	...
	panels.get_node("Panel" + str(global.store.selectedW+1)).get_node('Button').text = "Selected"
	panels.get_node("Panel" + str(global.store.selectedW+1)).get_node('Button').add_to_group("selectedA")
